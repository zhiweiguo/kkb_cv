...
from utils.regularization import DropBlock2D, LinearScheduler
from utils.activation import Swish, Mish
import matplotlib.pyplot as plt
import matplotlib.patches as patches


def create_modules(module_defs, active_mode=0, conv_mode=0):
    """
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        # 标准卷积
        if module_def["type"] == "convolutional" and conv_mode == 0:
        ...
        # depthwise conv
        elif module_def["type"] == "convolutional" and conv_mode == 1:
            bn = int(module_def["batch_normalize"])
            filters = int(module_def["filters"])    # 输出channel个数
            kernel_size = int(module_def["size"])
            pad = (kernel_size - 1) // 2
            # depthwise conv
            modules.add_module(
                f"dw_conv_{module_i}", # a newer formatting method for python3.x, called f-string. Better than %s..
                nn.Conv2d(
                    in_channels=output_filters[-1],
                    out_channels=output_filters[-1], # 通道数不变
                    kernel_size=kernel_size,
                    stride=int(module_def["stride"]),
                    padding=pad,
                    groups=output_filters[-1],  # 每个通道一组
                    bias=not bn,
                ),
            )
            if bn:
                modules.add_module(f"dw_batch_norm_{module_i}", nn.BatchNorm2d(output_filters[-1], momentum=0.9, eps=1e-5))
            if module_def["activation"] == "leaky":     # .cfg中有linear activation，说明linear啥也不干
                if active_mode == 0:
                    modules.add_module(f"dw_leaky_{module_i}", nn.LeakyReLU(0.1))
                elif active_mode == 1:
                    modules.add_module(f"dw_swish_{module_i}", Swish())
                elif active_mode == 2:
                    modules.add_module(f"dw_mish_{module_i}", Mish())
            # pointwise conv
            modules.add_module(
                f"pw_conv_{module_i}", # a newer formatting method for python3.x, called f-string. Better than %s..
                nn.Conv2d(
                    in_channels=output_filters[-1],
                    out_channels=filters, # 通道数改变
                    kernel_size=1,
                    stride=1,
                    padding=0,
                    groups=1, 
            ...
            if bn:
                modules.add_module(f"pw_batch_norm_{module_i}", nn.BatchNorm2d(filters, momentum=0.9, eps=1e-5))
            if module_def["activation"] == "leaky":     # .cfg中有linear activation，说明linear啥也不干
                if active_mode == 0:
                    modules.add_module(f"pw_leaky_{module_i}", nn.LeakyReLU(0.1))
                elif active_mode == 1:
                    modules.add_module(f"pw_swish_{module_i}", Swish())
                elif active_mode == 2:
                    modules.add_module(f"pw_mish_{module_i}", Mish())
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416, regu_mode=0, active_mode=0, conv_mode=0):
        ...
        self.hyperparams, self.module_list = create_modules(self.module_defs, self.active_mode, self.conv_mode)
        self.yolo_layers = [layer[0] for layer in self.module_list if hasattr(layer[0], "metrics")] # not used
        self.img_size = img_size
        self.seen = 0
        self.header_info = np.array([0, 0, 0, self.seen, 0], dtype=np.int32)
        # 添加dropblock
        if regu_mode == 2 or regu_mode == 3:
