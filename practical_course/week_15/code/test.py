...
def evaluate(model, path, iou_thres, conf_thres, nms_thres, img_size, batch_size):
    model.eval()
    # Get dataloader
    dataset = ListDataset(path, img_size=img_size, augment=0, multiscale=False)
    ...
    for batch_i, (imgs, targets) in enumerate(tqdm.tqdm(dataloader, desc="Detecting objects")):
        ...
        res = get_batch_statistics(outputs, targets, iou_threshold=iou_thres)
        if res:
            sample_metrics += res
...
if __name__ == "__main__":
    ...
    # Initiate model
    model = Darknet(opt.model_def, regu_mode=opt.regu_mode, active_mode=opt.active_mode).to(device)
